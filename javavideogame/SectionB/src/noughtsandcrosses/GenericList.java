...
public class GenericList<T> implements GenericListInterface<T>{
	...
	public void add(int pos, T item) throws ListIndexOutOfBoundsException{
		if (pos > numItems + 1 || pos < 1) {
			throw new ListIndexOutOfBoundsException("Out of bounds");
		} else {
			if (pos == 1) {
				// Making new element the head of the list
				ListNode<T> newHead = new ListNode<T>(item, head);
				head = newHead;
			} else {
				// Get the previous node
				ListNode<T> prev = find(pos-1);
				// Set the next of the new node to next of the prev
				ListNode<T> newCurrent = new ListNode<T>(item, prev.getNext());
				prev.setNext(newCurrent);
			}
