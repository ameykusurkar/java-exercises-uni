...
public class GameTree implements GameTreeInterface {
	...
	private int sizeTree(GameTreeNode node){
		int total = 0;
		// If it has no children, function will terminate
		for (GameTreeNode t : node.getChildren()) {
			total++;
			total += sizeTree(t);
		}
		return total;
	...
	private void expandTree(GameTreeNode node){
		Board board = node.getBoard();
	 	if (!board.isFinished()) {
	 		char newMark = board.getTurn();
	 		for (int i = 1; i <= 9; i++) {
	 			try {
	 				Board newBoard = board.makeCopy();
	 				newBoard.setMark(i, newMark);
	 				newBoard.setLastMarkPosition(i);
	 				node.getChildren().add(node.numberOfChildren()+1, new GameTreeNode(newBoard));
