...
public class Graph implements GraphInterface{
	...
	private void setIncomingDegreeRecursively(GraphNode node) {
		...
		if (node != null) {
			...
			// Sets degree of node's outgoing edges recursively
	...
	private QueueInterface<GraphNode> topologicalSort( ) {
		...
		QueueInterface<GraphNode> temp = new Queue<GraphNode>();
		QueueInterface<GraphNode> result = new Queue<GraphNode>();
		
		temp.enqueue(start);
		
		while (!temp.isEmpty()) {
			
			GraphNode curr = temp.dequeue();
			result.enqueue(curr);
			
			for (GraphEdge edge : curr.getOutgoingEdges()) {
				
				GraphNode child = edge.getEndPoint();
				
				// Decrements the degree of its child node
				child.setDegree(child.getDegree() - 1);
				
				// Adds child to temp queue if it has degree 0
	...
	private void computeEarliestCompletionTime( ) {
		...
		while (!sortedNodes.isEmpty()) {
			...
			GraphNode curr = sortedNodes.dequeue();
			
			curr.setEarliestCompletionTime(getEC(curr));
		
		}
	
	}
	
	private int getEC(GraphNode node) {
		
		int maxEC = 0;
		
		for (GraphEdge edge : node.getIncomingEdges()) {
			
			int temp = edge.getTaskDuration() +
					edge.getStartPoint().EarliestCompletionTime();
			
			if (temp > maxEC) maxEC = temp;
		}
